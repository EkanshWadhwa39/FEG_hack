...
import { createSandboxBrowserRequester } from "../src/bounded-browser-requester.js";
...
test('replacement preparation drains non-cooperative old workers: global concurrency stays at two', async () => {
  ...
  const requestAsset = async (url, options) => {
    ...
    active--; options.onBytes?.(4); return { completed: true, bodyBytes: 4 };
  };
...
test("synchronously cancelled preparation/launch never invokes a deferred authorization endpoint", async () => {
  let checks = 0;
  const authorization = { subscribe: () => () => {}, isGranted: () => true,
    async check() { checks++; return "GRANTED"; } };
  const f = setup({ authorization });
  try {
    const preparation = prepare(f.loader, "title-01"); f.loader.setEnabled(false);
    assert.notEqual((await preparation).status, "REQUESTS_COMPLETE");
    const launch = f.loader.beginLaunch({ ...variant, gameId: "title-01" }); f.loader.cancelLaunch();
    assert.equal((await launch).status, "AUTHORIZATION_BLOCKED");
    assert.equal(checks, 0); assert.equal(f.urls.length, 0);
    assert.equal(f.loader.snapshot().reservedBodyBytes, 0);
  } finally { f.loader.dispose(); }
});

test("foreground grant waits for both cancelled bodies to finish cleanup", { timeout: 2000 }, async () => {
  const cancels = []; let starts = 0;
  const requestAsset = createSandboxBrowserRequester({ origin, fetchImpl: async () => {
    starts++;
    return new Response(new ReadableStream({ cancel() { return new Promise(resolve => cancels.push(resolve)); } }));
  } });
  const f = setup({ requestAsset });
  try {
    const preparation = prepare(f.loader, "title-01"); await tick(); assert.equal(starts, 2);
    let settled = false;
    const launch = f.loader.beginLaunch({ ...variant, gameId: "title-20" }).then(r => { settled = true; return r; });
    assert.equal((await preparation).status, "CANCELLED"); await tick();
    assert.equal(cancels.length, 2); assert.equal(settled, false);
    cancels[0](); await tick(); assert.equal(settled, false);
    cancels[1](); const grant = await launch;
    assert.equal(grant.status, "LAUNCH_AUTHORIZED"); assert.equal(grant.signal.aborted, false);
    assert.equal(starts, 2); assert.equal(f.loader.snapshot().reservedBodyBytes, 114688);
    assert.equal(f.loader.snapshot().completedObjects, 0);
  } finally { cancels.forEach(resolve => resolve()); f.loader.dispose(); }
});

test("hung cleanup blocks launch and replacement by deadline without recycling reservations", { timeout: 2000 }, async () => {
  const cancels = []; let starts = 0;
  const requestAsset = createSandboxBrowserRequester({ origin, fetchImpl: async () => {
    starts++;
    return new Response(new ReadableStream({ cancel() { return new Promise(resolve => cancels.push(resolve)); } }));
  } });
  const f = setup({ requestAsset, operationTimeoutMs: 20 });
  try {
    const preparation = prepare(f.loader, "title-01"); await tick(); assert.equal(starts, 2);
    assert.equal((await f.loader.beginLaunch({ ...variant, gameId: "title-20" })).status, "LAUNCH_BLOCKED");
    assert.equal((await preparation).status, "CANCELLED");
    f.loader.resumeBrowsing();
    assert.equal((await prepare(f.loader, "title-02")).status, "REQUEST_DRAIN_BLOCKED");
    assert.equal(starts, 2); assert.equal(f.loader.snapshot().reservedBodyBytes, 114688);
    assert.equal(f.loader.snapshot().completedObjects, 0);
  } finally { cancels.forEach(resolve => resolve()); f.loader.dispose(); await requestAsset.waitForIdle(); }
});

test("authorization revocation while waiting for cleanup cannot issue a late launch grant", { timeout: 2000 }, async () => {
  const cancels = [];
  const requestAsset = createSandboxBrowserRequester({ origin, fetchImpl: async () =>
    new Response(new ReadableStream({ cancel() { return new Promise(resolve => cancels.push(resolve)); } })) });
  const f = setup({ requestAsset });
  try {
    const preparation = prepare(f.loader, "title-01"); await tick();
    const launch = f.loader.beginLaunch({ ...variant, gameId: "title-20" });
    await preparation; await tick(); f.authorization.setState("DENIED");
    assert.notEqual((await launch).status, "LAUNCH_AUTHORIZED");
    cancels.forEach(resolve => resolve()); await requestAsset.waitForIdle();
    assert.equal(f.loader.snapshot().completedObjects, 0);
  } finally { cancels.forEach(resolve => resolve()); f.loader.dispose(); }
});

test("completion metadata must agree with measured callbacks and a bounded safe-integer bodyBytes", async () => {
  for (const bodyBytes of [undefined, null, -1, 0.5, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1, 3, 200000]) {
    const requestAsset = async (_url, { onBytes }) => { onBytes(4); return { completed: true, bodyBytes }; };
    requestAsset.validateUrl = createSandboxBrowserRequester({ origin }).validateUrl;
    const f = setup({ requestAsset });
    try {
      assert.equal((await prepare(f.loader, "title-01")).status, "CANCELLED");
...
test("live state is rechecked on delivered bytes even without an environment subscription", async () => {
  let effectiveType = "4g"; let starts = 0;
  const requestAsset = async (_url, { onBytes }) => {
    starts++; effectiveType = "3g"; onBytes(4); return { completed: true, bodyBytes: 4 };
  };
  requestAsset.validateUrl = createSandboxBrowserRequester({ origin }).validateUrl;
  const f = setup({ requestAsset, environment: { read: () => ({ saveData: false, effectiveType, visibilityState: "visible" }) } });
  try {
    assert.equal((await prepare(f.loader, "title-01")).status, "CANCELLED");
    assert.equal(starts, 1); assert.equal(f.loader.snapshot().observedBodyBytes, 4);
    assert.equal(f.loader.snapshot().completedObjects, 0);
    assert.equal(f.loader.snapshot().reservedBodyBytes, 114688);
  } finally { f.loader.dispose(); }
});
