...
function bounded(operation, signal, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    Promise.resolve().then(() => {
      if (settled || signal.aborted) return; // Never invoke a cancelled endpoint.
      return operation();
 ...
 * with validateUrl(url), per-chunk onBytes callbacks and {completed:true,bodyBytes}.
 * Optional requestAsset.waitForIdle({signal}) includes asynchronous cancellation
 * cleanup; legacy adapters must keep their request promise pending through cleanup.
 * authorization.check({purpose,signal}) + isGranted() + required subscribe(listener),
 * environment.read() + optional subscribe(listener).
...
export function createContentLoader({ catalogue, session, prior, manifestSource, requestAsset,
  ...
  async function drain(controller) {
    await bounded(async () => {
      await Promise.allSettled([...pendingRequests]);
      // Bounded requester rejects promptly on abort while retaining slots until
      // full cleanup. Do not grant foreground launch during that cleanup window.
      if (!controller.signal.aborted && typeof requestAsset.waitForIdle === "function") {
        await requestAsset.waitForIdle({ signal: controller.signal });
      }
  ...
  async function resolve(target, controller) {
    const manifest = await bounded(() => manifestSource.resolve(target, { signal: controller.signal }),
      controller.signal, operationTimeoutMs);
    const validateReleaseAsset = manifestSource.validateReleaseAsset;
    return resolvePreparationIdentity(manifest, target, {
      validateUrl: url => requestAsset.validateUrl(url),
      validateReleaseAsset: typeof validateReleaseAsset === "function"
        ? (asset, identity) => validateReleaseAsset.call(manifestSource, asset, identity)
        : undefined,
    });
  }
        ...
        concurrency: 2, requestAsset: async (url, options) => {
          ...
          const work = (async () => {
            ...
            const response = await requestAsset(url, { ...options, onBytes: count => {
              if (!Number.isSafeInteger(count) || count < 0) { overrun = true; cancel(); controller.abort(); return; }
              observedBodyBytes += count; assetBytes += count;
              if (!Number.isSafeInteger(observedBodyBytes) || !Number.isSafeInteger(assetBytes)
                  || assetBytes > options.estimatedBytes) { overrun = true; cancel(); controller.abort(); }
              if (!isGranted() || !eligibility().allowed) controller.abort();
            } });
            ...
            if (!Number.isSafeInteger(response.bodyBytes) || response.bodyBytes < 0
                || response.bodyBytes > options.estimatedBytes || response.bodyBytes !== assetBytes) {
              overrun = true; cancel(); controller.abort(); throw new Error("BODY_ACCOUNTING_INVALID");
            }
            if (!isGranted() || !eligibility().allowed) { controller.abort(); throw new Error("GOVERNOR_REVOKED"); }
            completed.add(url);
          })();
